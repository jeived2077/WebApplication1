...
function applyFilters() {
    ...
    if (directorFilter && directorFilter !== "Нету данных") {
        filteredMovies = filteredMovies.filter(movie => movie.directors.includes(directorFilter));
    }
...
function resetForm() {
    ...
    uploadedPoster = null; // Сбрасываем загруженный постер
    document.getElementById("movie-id").value = "";
    document.getElementById("title").value = "";
    document.getElementById("year").value = "";
    document.getElementById("description").value = "";
    posterInput.value = ""; // Очищаем input
    posterStatus.textContent = ""; // Очищаем статус
...
document.getElementById("genre-modal").addEventListener("change", async function(e) {
        ...
        if (newGenre) {
            try {
                ...
                if (response.ok) {
                    await loadMovies();
                    resetForm();
                    document.getElementById("genre-modal").value = newGenre;
                } else {
        ...
        if (firstName && lastName) {
            ...
            try {
                ...
                if (response.ok) {
                    await loadMovies();
                    resetForm();
                    document.getElementById("director-modal").value = newDirector;
                } else {
        ...
        if (firstName && lastName) {
            ...
            try {
                ...
                if (response.ok) {
                    await loadMovies();
                    resetForm();
                    const actorsSelect = document.getElementById("actors-modal");
                    Array.from(actorsSelect.options).forEach(option => {
                        if (option.value === newActor) option.selected = true;
                    });
                } else {
...
function populateMovieTable() {
    ...
    allMovies.forEach(movie => {
        ...
        row.addEventListener("click", () => editMovie(movie.id));
...
function editMovie(movieId) {
    ...
    if (movie) {
        ...
        uploadedPoster = movie.poster; // Устанавливаем текущий постер как базовый
